...
/* Analysis-only batched decoding through the existing describe protocol. */
char *ixyk_analysis_decode(const unsigned char *bytes, unsigned length) {
    xed_decoded_inst_t inst;
    xed_decoded_inst_zero(&inst);
    xed_decoded_inst_set_mode(&inst, XED_MACHINE_MODE_LONG_64, XED_ADDRESS_WIDTH_64b);
    if (!length || length > 15 || xed_decode(&inst, bytes, length) != XED_ERROR_NONE || xed_decoded_inst_get_length(&inst) != length) return NULL;
    char *json = NULL;
    size_t size = 0;
    FILE *out = open_memstream(&json, &size);
    if (!out) return NULL;
    describe(out, &inst, bytes, length);
    if (fclose(out)) { free(json); return NULL; }
    return json;
}
