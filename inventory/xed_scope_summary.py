...
def main():
    ...
    opcodes=collections.defaultdict(set);forms=collections.defaultdict(collections.Counter);missing=set()
    for line in a.sites.read_text().splitlines():
        family,_,address,h,opcode=line.split('\t')
        if h not in matches:missing.add(h);continue
        m=matches[h];counts[family][m['status']]+=1;opcodes[family].add(opcode);forms[family][m.get('form','decode_error')]+=1
    ...
    result={'scope':'Candidate envelopes for valid family inputs; all record modes retained. Source admission, target exhaustiveness, and domain restriction are proof obligations. Counts are applicability, not completed proofs.',
            'families':{f:{'sites':sum(c.values()),'opcode_mnemonics':sorted(opcodes[f]),'required_forms':dict(sorted(forms[f].items())),'status_counts':dict(c),'gaps':[{'status':s,'form':form,'sites':n} for (s,form),n in gaps[f].most_common()]} for f,c in sorted(counts.items())}}
